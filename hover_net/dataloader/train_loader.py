...
class FileLoader(torch.utils.data.Dataset):
    ...
    def setup_augmentor(self, worker_id, seed):
        ...
        self.shape_augs_img = iaa.Sequential(self.augmentor[0])
        self.shape_augs_mask = iaa.Sequential(self.augmentor[1])
    ...
    def __getitem__(self, idx):
        ...
        if self.shape_augs_img is not None:
            self.shape_augs_img.localize_random_state_()
            shape_augs_mask = self.shape_augs_mask.copy_random_state(self.shape_augs_img, matching="name")
            shape_augs_mask = shape_augs_mask.to_deterministic()
            shape_augs_img = self.shape_augs_img.to_deterministic()

            img = shape_augs_img.augment_image(img)
            ann = shape_augs_mask.augment_image(ann)
    ...
    def __get_augmentation(self, mode, rng):
        if mode == "train":
            shape_augs_img = [
                ...
                iaa.CropToFixedSize(
                    ...
                    position="uniform",
                    seed=rng,
                    name="train_randomcrop"
                ),
                iaa.Fliplr(0.5, seed=rng, name="train_fliplr"),
                iaa.Flipud(0.5, seed=rng, name="train_flipup"),
            ]
        ...
        return shape_augs_img, shape_augs_mask, input_augs
