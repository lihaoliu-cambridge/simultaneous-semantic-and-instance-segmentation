...
class RunManager(object):
    ...
    ####
    def _run_once(self, opt, run_engine_opt, log_dir, prev_log_dir=None, phase_idx=0):
        """
        ...
        def get_last_chkpt_path(prev_phase_dir, net_name):
            ...
            last_chkpts_path = (
                f"{prev_phase_dir}/"
                f"{net_name}_epoch={max(epoch_list)}.tar"
            )
        ...
        for net_name, net_info in net_info_opt.items():
            ...
            if pretrained_path is not None:
                if pretrained_path == -1:
                    ...
                    if chkpt_ext == "npz":
                        ...
                        net_state_dict = torch.load(pretrained_path)["desc"]
                        special_train_flag = True
                        optimizer_state_dict = torch.load(pretrained_path)["optimizer"]
                        lr_scheduler_state_dict = torch.load(pretrained_path)["lr_scheduler"]
                ...
                print("Loading again:", pretrained_path)
            ...
            if special_train_flag:
                ...
                torch.cuda.empty_cache()
                print("Loading previous lr_scheduler.")
    ...
    def run(self):
        ...
        for phase_idx, phase_info in enumerate(phase_list):
            ...
            # # If you don't want to re-train phase 1, denote this part.
            # if phase_idx == 0:
            ...
            self._run_once(
                ...
                prev_log_dir=prev_save_path, phase_idx=phase_idx)
            prev_save_path = save_path
            self.phase_idx += 1
